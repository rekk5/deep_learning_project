...
class DualDRModel(nn.Module):
    def __init__(self, num_classes=5, pretrained=True):
        ...
        # Load two pretrained DenseNet201 backbones
        self.backbone1 = models.densenet201(pretrained=pretrained)
        self.backbone2 = models.densenet201(pretrained=pretrained)
        
        # Get the number of features (DenseNet201 has 1920 features)
        num_features = self.backbone1.classifier.in_features
...
def main():
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # DenseNet typically uses 224x224
    ...
    # Separate validation transform (no augmentation)
    ...
    best_aptos_kappa = train_model(
        ...
        APTOS_EPOCHS, 'aptos_model_dual.pth'
    )
    # Rename the plot for APTOS training
    os.rename('training_metrics.png', 'aptos_training_metrics.png')
    ...
    model.load_state_dict(torch.load('aptos_model_dual.pth', weights_only=True))
    ...
    best_deepdrid_kappa = train_model(
        ...
        DEEPDRID_EPOCHS, 'final_model_dual.pth'
    )
    # Rename the plot for DeepDRiD training
    os.rename('training_metrics.png', 'deepdrid_training_metrics.png')
    ...
    model.load_state_dict(torch.load('final_model_dual.pth', weights_only=True))
    model.eval()
